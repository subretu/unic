...
class TestConvertUnixtime:
    def test_convert_unixtime_utc(self):
        ...
        result = test_timeobject.convert_unixtime("2022-07-18 13:49:00")

        assert result == 1658152140

    def test_convert_unixtime_except_milisecond_utc(self):
        test_timeobject = timeobject.TimeObject()
        result = test_timeobject.convert_unixtime("2022-07-18 13:49:00.123")

        assert result == 1658152140123

    def test_convert_unixtime_jst(self):
        test_timeobject = timeobject.TimeObject()
        result = test_timeobject.convert_unixtime("2022-07-18 13:49:00", tz="Asia/Tokyo")
    ...
    def test_convert_unixtime_parameter_number_error(self):
        with pytest.raises(Exception) as e:
            test_timeobject = timeobject.TimeObject()
            _ = test_timeobject.convert_unixtime("2022-07-18 13:49:00", tz="Asia/Tokyo", hoge=123456)
    ...
    def test_convert_unixtime_parameter_error(self):
        with pytest.raises(Exception) as e:
            test_timeobject = timeobject.TimeObject()
            _ = test_timeobject.convert_unixtime("2022-07-18 13:49:00", timezone="Asia/Tokyo")
